...
from models  import iBEAt_DWI_monoexponential
...
def iBEAt_test_DWI(Elastix_Parameter_file_PATH, output_dir, slice_sorted_acq_time, original_images, image_parameters, fname, lstFilesDCM):
    ...
    # select model
    MODEL = [iBEAt_DWI_monoexponential,'fitting']
    ...
    elastix_model_parameters = elastixImageFilter.ReadParameterFile(Elastix_Parameter_file_PATH + "/BSplines_IVIM.txt")
    ...
    for i in range(shape[2]):
        ...
        im_motion_corrected.save(output_dir + '/coregistered/MDR-registered_IVIM_'+ str(i) + ".tiff")
    ...
    S0_DWI = np.reshape(fitted_quantitative_Params[::2],[shape[0],shape[1]]) 
    S0_DWI_Img = Image.fromarray(S0_DWI)
    S0_DWI_Img.save(output_dir + '/fitted_parameters/DWI_S0' + ".tiff")
    
    ADC = np.reshape(fitted_quantitative_Params[1::2],[shape[0],shape[1]]) 
    ADC_Img = Image.fromarray(ADC)
    ADC_Img.save(output_dir + '/fitted_parameters/ADC' + ".tiff")
    
    diagnostics.to_csv(output_dir + 'DWI_largest_deformations.csv')

    print("Finished processing Model Driven Registration case for iBEAt study IVIM/DWI sequence!")
