...
def IVIM_fitting_pixel(args):
    ...
    par = np.empty((3, 2))
    try:
        par[0, :], _ = curve_fit(func, xdata=b, ydata=S[:nb], p0=p0, bounds=(
            lb, ub), method='trf', maxfev=500)
    except RuntimeError:
        par[0, :] = p0
    try:
        par[1, :], _ = curve_fit(func, xdata=b, ydata=S[nb:2 *
                                                        nb], p0=p0, bounds=(lb, ub), method='trf', maxfev=500)
    except RuntimeError:
        par[1, :] = p0
    try:
        par[2, :], _ = curve_fit(
            func, xdata=b, ydata=S[2 * nb:], p0=p0, bounds=(lb, ub), method='trf', maxfev=500)
    except RuntimeError:
        par[2, :] = p0
