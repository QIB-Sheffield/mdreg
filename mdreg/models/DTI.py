...
def DTI_fitting(im, b, thresh_val, method='linear'):
    ...
    for i in range(npixels):
        ...
        # DiffusionTensor=[Mi[0] Mi[1] Mi[2]; Mi[1] Mi[3] Mi[4]; Mi[2] Mi[4]
        # Mi[5]]
        ...
        if np.sum(EigenValues) != 0:
            EigenValues, EigenVectors = zip(
                *sorted(zip(EigenValues, EigenVectors)))
        ...
        if denominator == 0:
            ...
            FA_mask[i] = np.sqrt(1.5) * (np.sqrt((EigenValues[0] - ADCv)**2 + (
                EigenValues[1] - ADCv)**2 + (EigenValues[2] - ADCv)**2) / denominator)
        ADC_mask[i] = ADCv
...
def main(images_to_be_fitted, signal_model_parameters):
    ...
    for idx_b in range(len(b_values)):
        b[:, :, idx_b] = np.outer(
            np.outer(b_values[idx_b], bVec[idx_b, :].T), bVec[idx_b, :])
