...
class Folder(Database):
    ...
    def __init__(self, path=None, status=StatusBar(), dialog=Dialog()):
        ...
        """
        self.__dict__['dataframe'] = pd.DataFrame(
            [] * len(self._columns), columns=self._columns)
    ...
    def scan(self):
        ...
        """
        files = [
            item.path for item in utilities.scan_tree(
                self.path) if item.is_file()]
        self.__dict__['dataframe'] = utilities.dataframe(
            files, self._columns, self.status)
        ...
        for i, patient in enumerate(self.children()):
            ...
            for j, study in enumerate(patient.children()):
                ...
                for k, series in enumerate(study.children()):
                    print(
                        '            SERIES [' + str(k) + ']: ' + series.label())
                    print('                Nr of instances: ' +
                          str(len(series.children())))
    ...
    def _append(self, ds, checked=False):
        ...
        row['removed'] = False
        row['created'] = True
        self.__dict__['dataframe'] = self.dataframe.append(
            row)  # REPLACE BY CONCAT
    ...
    def _multiframe_to_singleframe(self):
        ...
        if nr_multiframe != 0:
            ...
            for filepath, instance in self.dataframe[multiframe].iterrows():
                ...
                #
                singleframe_files = utilities.split_multiframe(
                    filepath, str(instance.SeriesInstanceUID))
                if singleframe_files != []:
                    ...
                    self.__dict__['dataframe'] = pd.concat(
                        [self.dataframe, df])
                    # delete the original multiframe
                    # CHANGE THIS - mark for removal but don't delete
                    # show enhanced DICOM in Tree but set the class to block
                    # display etc.
                    os.remove(filepath)
