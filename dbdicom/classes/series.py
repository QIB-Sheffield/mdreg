...
class Series(Record):
    ...
    def map_onto(self, target):
        ...
        if in_memory:
            mapped_series.read()
        # create record.images() to return children of type image
        target_images = target.children()
        for i, target_image in enumerate(target_images):
            target_image.read()
            self.status.progress(i, len(target_images))
            pixel_array = np.zeros(
                (target_image.Rows, target_image.Columns), dtype=np.bool)
            for j, source_image in enumerate(source_images):
                ...
                if not in_memory:
                    source_image.read()
                array = source_image.map_onto(
                    target_image).array().astype(np.bool)
    ...
    def export_as_nifti(self, directory=None, filename=None):
        ...
        dicomHeader = nib.nifti1.Nifti1DicomExtension(
            2, self.instances(0).read())
