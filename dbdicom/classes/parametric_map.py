...
class ParametricMap(object):
    def selectParametricMap(self, dicom, imageArray, argument):
        ...
        method = getattr(
            self,
            methodName,
            lambda: "No valid Parametric Map chosen")
    ...
    def RGB(self, dicom, imageArray):
        ...
        dicom.WindowCenter = int(
            (np.amax(imageArray) - np.amin(imageArray)) / 2)
        dicom.WindowWidth = np.absolute(
            int(np.amax(imageArray) - np.amin(imageArray)))
    ...
    def ADC(self, dicom, imageArray):
        ...
        # Need to have a better look at this
        dicom.PixelAspectRatio = ["1", "1"]
        dicom.RescaleSlope = 1
        dicom.RescaleIntercept = 0
        ...
        dicom.RealWorldValueMappingSequence = [
            Dataset(), Dataset(), Dataset(), Dataset()]
        dicom.RealWorldValueMappingSequence[0].QuantityDefinitionSequence = [
            Dataset(), Dataset()]
        dicom.RealWorldValueMappingSequence[0].QuantityDefinitionSequence[0].ValueType = "CODE"
        dicom.RealWorldValueMappingSequence[0].QuantityDefinitionSequence[1].ConceptCodeSequence = [
            Dataset(), Dataset(), Dataset()]
        dicom.RealWorldValueMappingSequence[0].QuantityDefinitionSequence[1].ConceptCodeSequence[0].CodeValue = "113041"
        dicom.RealWorldValueMappingSequence[0].QuantityDefinitionSequence[
            1].ConceptCodeSequence[1].CodingSchemeDesignator = "DCM"
        dicom.RealWorldValueMappingSequence[0].QuantityDefinitionSequence[
            1].ConceptCodeSequence[2].CodeMeaning = "Apparent Diffusion Coefficient"
        dicom.RealWorldValueMappingSequence[1].MeasurementUnitsCodeSequence = [
            Dataset(), Dataset(), Dataset()]
        ...
        saveAnatomicalInfo(
            anatomyString,
            dicom.RealWorldValueMappingSequence[3])

        return

    def T2Star(self, dicom, imageArray):
        # find a mechanism to pass reconstruct pixel here
    ...
    def SEG(self, dicom, imageArray):
        ...
        # The commented parts are to apply when we decide to include SEG IOD.
        # No readers can deal with this yet
        ...
        pixelArray = np.transpose(imageArray.astype(
            np.uint8))  # Should we multiply by 255?
        dicom.PixelData = pixelArray.tobytes()
        ...
        if hasattr(dicom, "ImageComments"):
            dicom.ContentDescription = dicom.ImageComments.split(
                '_')[-1]  # 'Image segmentation'
            ...
            for key in segment_dictionary:
                ...
                dicom.SegmentSequence[2].SegmentDescription = str(
                    segment_dictionary[key])
                dicom.SegmentSequence[3].SegmentLabel = str(
                    segment_dictionary[key])
