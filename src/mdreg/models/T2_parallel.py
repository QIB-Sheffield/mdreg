...
def T2_fitting_pixel(parallel_arguments):
    ...
    try:
        popt, pcov = curve_fit(exp_func, xdata = T2_prep_times, ydata = images_to_be_fitted[pixel_index,:], p0=[np.max(images_to_be_fitted[pixel_index,:]), 80], bounds=(lb,ub), method='trf',maxfev=10000)  
        S0x = popt[0] 
        T2x = popt[1]
    except:
