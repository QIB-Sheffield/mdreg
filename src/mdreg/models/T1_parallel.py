...
def T1_fitting_pixel(parallel_arguments):
    ...
    try:
        popt, pcov = curve_fit(exp_func, xdata = inversion_times, ydata = images_to_be_fitted[pixel_index,:], p0 =[np.max(images_to_be_fitted[pixel_index,:]), np.max(images_to_be_fitted[pixel_index,:])*1.9, 1500*(1.9-1)] , bounds = (lb, ub), method = 'trf', maxfev=10000)
        ax =  popt[0] 
        bx =  popt[1] 
        T1_apparent_x = popt[2]
    except:
        #if model does not converge fill results with starting values
        ax =  np.max(images_to_be_fitted[pixel_index,:])        #S0
        bx =  np.max(images_to_be_fitted[pixel_index,:])*1.9    #S0 * alpha
        T1_apparent_x = 1500*(1.9-1)                            #T1 * (alpha-1)
    fitx = []
